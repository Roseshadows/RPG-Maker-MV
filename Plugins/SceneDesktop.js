...
/*:
 * @plugindesc ver1.50 - 电脑/手机桌面样式界面的父类。单用没有任何效果。
 ...
 * 
 * ver1.00 - 完成。
 * ver1.50 - 更新了drawIconEx()方法，新增了参数，
 *           允许另用一张非32x32的图标集作为app图标。
 * 
 */
 ...
 * @param {string} fileName 图标源文件名
 * @param {number} w 单个图标宽度
 * @param {number} h 单个图标高度
 */

Window_Desktop.prototype.drawIconEx = function(iconIndex, x, y, dw, dh, fileName='IconSet', w=Window_Base._iconWidth, h=Window_Base._iconHeight) {
...
Window_Desktop.prototype.drawItem = function(index) {
    ...
    this.drawItemName(index, rect);
}

Window_Desktop.prototype.drawItemInitStyle = function(index) {
    this.changeTextColor(this.normalColor());
    this.changePaintOpacity(this.isCommandEnabled(index));
}

Window_Desktop.prototype.drawItemIcon = function(index, rect) {
...
Window_Desktop.prototype.drawItemName = function(index, rect) {
 ...
 * @param {boolean} visible 图片是否可见
 * @param {number} rotate 图片旋转角度
 * @param {number} blendMode 图片混合模式
 * @param {number} scaleX 图片缩放X
 * @param {number} scaleY 图片缩放Y
 */
