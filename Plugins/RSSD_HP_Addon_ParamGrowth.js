...
(function(_){
    ...
    var temp_arr = JSON.parse(_.parameters['Growth Type List']) || [];
    _.growthList = {};
    temp_arr.forEach((type)=>{
        var growth = JSON.parse(type);
        var obj = {};
        var flag = growth['Flag'];
    ...
    temp_arr2.forEach((config)=>{
        ...
        var s_name = configs['Short Name'];
        var flag = configs['Flag'];
    ...
    if(Imported.RSSD_HP) {
        ...
        Game_Battler.prototype.refreshCusparamsGrowthType = function() {
            var keys = Object.keys(_.globalGrowthParams);
            for(var i = 0; i < keys.length; i++) {
                var s_name = keys[i];
                var flag = _.globalGrowthParams[s_name];
                this._cusparamsGrowthType[s_name] = flag;
            };
        };

        Game_Battler.prototype.cusparamDatabaseGrowthType = function(s_name) {
            var obj = this.isActor() ? this.actor() : this.enemy();
        ...
        Game_Actor.prototype.refreshCusparamsPrivateGrowthType = function() {
            ...
            for(var i = 0; i < keys.length; i++) {
                var s_name = keys[i];
                var flag = this.cusparamDatabaseGrowthType(s_name);
        ...
        Game_Actor.prototype.cusparamGrowthSumForLevel = function(s_name, level) {
            if(Object.keys(_.globalGrowthParams).contains(s_name)) {
                var flag = this.cusparamGrowthType(s_name);
                var group = _.growthList[flag].group;
                if(group.length < 2) {
        ...
        Game_Actor.prototype.cusparam = function(s_name) {
            var value = __Game_Actor_cusparam.call(this, s_name) + this.levelCusparam(s_name);
            var min = _.paramList[s_name].min || -Infinity;
            var max = _.paramList[s_name].max || Infinity;
                ...
                if(command.toLowerCase() === '::rssd_hp_pg') {
                    switch(args[0].toLowerCase()) {
                        case 'change':
                            ...
                            var flag = args[3].indexOf('growth') > 0 ? args[2].replace('growth[','').replace(']','') : null;
                            if(args[1].toLowerCase() === 'all_actors') {
                                $gameActors.changeCusparamGrowthTypeForAll(s_name, flag)
                            } else {
                                var actorId = args[1].indexOf('actor') > 0 ? +args[0].replace('actor[','').replace(']','') : 0;
                                var classId = args[1].indexOf('class') > 0 ? +args[0].replace('class[','').replace(']','') : 0;
                                var isActor = !!actorId && !classId;
                                if(isActor) {
                                    $gameActors.actor(actorId).changeCusparamGrowthType(s_name, flag);
                                } else {
                                    $gameActors._data.forEach((actor)=>{
